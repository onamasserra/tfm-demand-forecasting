...
\hline
XGBoost & 1.105 & 1.631 & 50.7 & 1.631 \\
Random Forest & 1.099 & 1.636 & 49.9 & 1.635 \\
LightGBM & 1.130 & 1.636 & 49.9 & 1.635 \\
...
The best-performing model is XGBoost, with an RMSE of 1.631, representing a 28.4\% improvement over the naive baseline (2.277). Random Forest and LightGBM follow closely (both at 1.636), making the three tree-based models virtually interchangeable in accuracy. LSTM (1.662) and ARIMA (1.734) also outperform the baseline substantially, though with a small gap behind the tree models.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/24_all_models_comparison.png}
\caption{Comparison of forecast accuracy (RMSE) across all six models. XGBoost achieves the lowest RMSE, with all ML models substantially outperforming the naive baseline.}
...
\textbf{Per-product RMSE distribution.} Analysis at the individual product level reveals that LightGBM achieves a median per-product RMSE of 1.32, compared to 1.83 for the naive baseline. The improvement is not uniform: some products see RMSE reductions exceeding 8 units, while approximately 10 products show marginally worse performance with ML than with the naive approach. These products tend to have very low and stable demand where the simple lag-7 heuristic is already near-optimal.
...
\caption{Total safety stock requirements by model and service level. The ML models maintain a consistent ~27\% advantage over the naive baseline across all service levels.}
...
\caption{Annual safety stock cost by model. The ML models reduce total costs by approximately 27\% compared to the naive baseline.}
...
\caption{Per-product safety stock distribution: Naive vs XGBoost. The ML model shifts the distribution towards lower safety stock requirements across the product portfolio.}
