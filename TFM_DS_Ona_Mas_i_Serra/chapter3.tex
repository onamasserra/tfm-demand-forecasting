...
All three tree-based machine learning models substantially outperform both the naive baseline and the deep learning alternative. XGBoost achieves the lowest RMSE (1.631), followed closely by Random Forest and LightGBM (both 1.636). The RMSE improvement over the naive baseline ranges from 23.9\% (ARIMA) to 28.4\% (XGBoost).

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/24_all_models_comparison.png}
\caption{Comparison of forecast accuracy (RMSE) across all six models. The three tree-based models form a tight cluster, with ARIMA and LSTM close behind, all substantially outperforming the naive baseline.}
\label{fig:all_models_comparison}
\end{figure}

ARIMA achieves an RMSE of 1.734 when fitted to all 861 products, placing it between the LSTM (1.662) and the naive baseline (2.277). This represents a 23.9\% improvement over the naive approach, demonstrating that classical time series methods remain competitive when applied at scale. The LSTM achieves the second-best RMSE (1.662), narrowing the gap with the tree-based models. This result is consistent with the findings of the M5 forecasting competition \citep{makridakis2021m5findings}, where gradient boosting algorithms consistently outperformed both classical statistical methods and standalone deep learning approaches, though the margins were often narrow.
