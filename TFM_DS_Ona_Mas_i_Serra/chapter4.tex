...
This project set out to develop machine learning models for daily demand forecasting in the automotive sector and to evaluate their impact on safety stock costs. The results confirm that tree-based ensemble methods (Random Forest, XGBoost, and LightGBM) substantially outperform both the naive lag-7 baseline and classical time series approaches, reducing forecast error by approximately 28\% and translating into annual safety stock cost savings of approximately 27\%.

The three tree-based models achieved virtually identical performance (RMSE between 1.633 and 1.637 on the last validation fold), indicating that the choice among them is less important than the decision to adopt ML-based forecasting over the naive approach. ARIMA and LSTM also outperformed the baseline, though with a smaller margin, confirming that the engineered features (lag values, rolling statistics, and EWMA) capture demand patterns more effectively than raw time series modelling alone.
...
The economic impact analysis, using real per-product cost parameters provided by the company (replenishment cycles, lead times, and unit prices), demonstrated that the ML models reduce total annual safety stock costs from approximately \euro11.4 million to \euro8.3 million, a saving of approximately \euro3.1 million per year. This result is robust across different service levels (90\%, 95\%, 99\%), with the relative advantage of ML models remaining consistent.
...
\begin{enumerate}
    ...
    \item \textit{Economic impact evaluation:} The safety stock cost analysis was conducted using real company data, demonstrating annual savings of approximately \euro3.1 million. This objective was fully achieved.
\end{enumerate}
...
Feature engineering matters more than model selection. The three tree-based models achieved nearly identical results despite using fundamentally different learning strategies (bagging vs gradient boosting). The shared factor was the same set of engineered features, particularly the smoothed demand history (EWMA, rolling statistics) and lag values. Investing time in understanding the data and crafting meaningful features yielded more value than fine-tuning model architectures. For example, the 28-day EWMA alone accounted for a mean absolute SHAP value of 0.473, more than the next four features combined.
...
Simple baselines are surprisingly strong. The naive lag-7 baseline, which simply repeats last week's sales, already captures the dominant weekly pattern in the data. The ML models improved on it by 28\%, but this required substantial effort in feature engineering, hyperparameter tuning, and validation design. In practice, the marginal value of ML over a well-chosen baseline should always be weighed against the implementation complexity.
